...
impl LineWrapper {
    ...
    pub fn should_truncate_line(
        ...
        match truncate_from {
            TruncateFrom::Start => {
                // For truncation from start, we need to iterate in reverse
                // Collect grapheme indices first, then iterate in reverse
                let grapheme_indices: Vec<(usize, &str)> = line
                    .grapheme_indices(true)
                    .collect();
                
                for (ix, grapheme) in grapheme_indices.iter().rev() {
            ...
            TruncateFrom::End => {
                // For truncation from end, iterate forward through graphemes
